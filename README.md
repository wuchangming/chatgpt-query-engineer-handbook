...
由于 `🤖️ChatGPT` 的回答是开放性和发散的，但对于具体的开发任务，更多是需要针对性的回答。为了提高 `ChatGPT 提问工程师` 在与 `🤖️ChatGPT` 合作编程中的效率，而创建了这份文档来收集和整理常用的`问题`。

该指南旨在帮助 `ChatGPT 提问工程师` 提高提问效率，欢迎 PR 👏👏👏 补充更多好问法。

_注意：本文档仅针对 `ChatGPT 提问工程师` 开发工作的场景。使用 `🤖️ChatGPT` 应该是开放性的，与 `🤖️ChatGPT` 进行无限可能的思维碰撞才能给人类带来颠覆性的应用 🌟。_

## 📃 文档使用说明

-   稳定性：强 中 弱

    -   由于 `🤖️ChatGPT` 的回答是发散的，稳定性代表回答符合期望的程度。
    -   尝试 10 次
...
-   明确处理的内容
    ...
    -   例子 2 ：有时候 🤖️ChatGPT 无法区分需要处理的内容是什么，可以加 「」{} 之类的把内容圈起来。
        ...
        「该指南旨在帮助 `ChatGPT 提问工程师` 提高提问效率」
        ```

    -   例子 3 ：实在不行也可以再加些描述，所有提问都可以按这个模式去处理。
...
-   ✨ 提问附加条件「重要概念」
    ...
    -   所有提问都是可以添加任意附加条件「或者说附加需求」。附加条件可以非常抽象，这也是 `🤖️ChatGPT` 和传统 API 调用最大的不同，也是最颠覆的地方 🚀。
    -   例子：
...
-   **设置简写**
    ...
    -   用途：很多时候，我们会有一些常用的内容，比如「重新回答这一问题」，这时可以用该问题设置简写，以后只需要发送简写就可以了。
    -   例子
        ```
...
-   **继续完成回答**
    ...
    -   用途：`🤖️ChatGPT` 回答太长的内容时经常会中断，可用该提问要求完成回答
